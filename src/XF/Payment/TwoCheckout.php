...
class TwoCheckout extends AbstractProvider
{
		...
		if ($input['credit_card_processed'] === 'Y')
		{
			...
			if ($purchase->recurring && $options['api_username'] && $options['api_password'])
			{
				try
				{
					...
					$sale = Utils::jsonDecode($saleResponse->getBody()->getContents(), true);
	...
	{
		$lineItemIds = Utils::jsonDecode($purchaseRequest->provider_metadata, true);
		...
		try
		{
			foreach ($lineItemIds AS $lineItemId)
			{
				...
				$cancellation = Utils::jsonDecode($cancelResponse->getBody()->getContents(), true);
