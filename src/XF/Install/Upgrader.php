...
class Upgrader
{
	...
	public function syncUpgradeLogStructure(): void
	{
		$sm = $this->db()->getSchemaManager();

		if (!$sm->columnExists('xf_upgrade_log', 'last_step'))
		{
			$sm->alterTable('xf_upgrade_log', function (Alter $table)
			{
	...
	public function assertValidUpgradeSteps(AbstractUpgrade $upgrade)
	{
	...
	{
		$new = Arr::arrayFilterKeys($config, [
			'db',
			'debug',
			'cookie',
			'enableMail',
	...
	public function enqueueFileCleanUp(): void
	{
		$this->app->jobManager()->enqueueUnique('xfFileCleanUp' . \XF::$versionId, FileCleanUp::class, [
			'addon_id' => 'XF',
		], false);
	}
	...
	public function enqueueStatsCollection()
	{
		...
		{
			$this->app->jobManager()->enqueueUnique('xfCollectStats', CollectStats::class, [], false);
		}
	}

	public function clearRegistryCache(): void
	{
		/** @var AdapterInterface|null $cache */
		$cache = $this->app->cache('registry', true, false, true);
		if (!$cache)
		{
			return;
		}
