...
use function count, in_array, intval, is_array, strlen;
...
trait InstallHelperTrait
{
	...
	{
		/** @var Phrase $phrase */
		$phrase = $this->app->em()->create(Phrase::class);
		$phrase->title = $title;
	...
	{
		/** @var Widget $widget */
		$widget = $this->app->em()->create(Widget::class);
	...
	/**
	 * Allows widget positions to be edited, appended or removed using
	 * $positions [position => order]
	 * To add a new position, simply add it to the $newPositions array
	 * with an appropriate order. To edit an existing position, do the same.
	 * To remove a position, set the order for that position to null.
	 *
	 * @param       $widgetKey
	 ...
	 */
	public function editWidgetPositions($widgetKey, array $newPositions)
	{
		/** @var Widget $widget */
		$widget = $this->app()->finder(WidgetFinder::class)
		...
		if ($widget)
		{
			...
			foreach ($newPositions AS $position => $order)
			{
				if ($order === null)
				{
			...
			$widget->set('positions', $positions);
			$widget->save(false);
		}
	}
	...
	{
		/** @var ActivitySummarySection $section */
		$section = $this->app->em()->create(ActivitySummarySection::class);
	...
	{
		$widget = $this->app->finder(WidgetFinder::class)->where('widget_key', $widgetKey)->fetchOne();
		if (!$widget)
		{
			return;
		}
	 ...
	 * @return mixed[]|bool
	 */
	public function removeCriteriaRules(
		string $tableName,
		array $primaryKeys,
	...
	)
	{
		...
		if (!isset($stepData['max']))
		{
			$stepData['max'] = $this->db()->fetchOne(
				"SELECT MAX({$primaryKeys[0]}) FROM {$tableName}"
			);
		}
		...
		$next = $this->removeCriteriaRulesFromResults(
			$results,
			$tableName,
			$primaryKeys,
			$criteriaField,
		...
		if ($next && count($primaryKeys) > 1)
		{
			// with a multi-column key, we assume the first column is an integer
			// and use that for ordering, but we might not finish all rows with
			// that first column value so fetch the rest of the last one that we
			// saw and convert them all here, otherwise they won't be converted
			...
			$this->removeCriteriaRulesFromResults(
				$results,
				$tableName,
				$primaryKeys,
				$criteriaField,
	 ...
	 * @param string[] $rules
	 */
	protected function removeCriteriaRulesFromResults(
		array $results,
		string $tableName,
		array $primaryKeys,
		string $criteriaField,
	...
	): int
	{
		...
		foreach ($results AS $result)
		{
			...
			$criteria = @json_decode($result[$criteriaField], true);
			if (!$criteria)
			{
				continue;
			}
			...
			{
				if (in_array($criterion['rule'], $rules, true))
				{
			...
			$this->db()->update(
				$tableName,
				[$criteriaField => json_encode(
					array_values($criteria),
					JSON_PARTIAL_OUTPUT_ON_ERROR
				)],
				implode(' AND ', $pkConditions),
				$pkValues
			);
		}
