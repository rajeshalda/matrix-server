...
class ForumController extends AbstractController
{
	...
	public function actionIndex(ParameterBag $params)
	{
		...
		{
			return $this->rerouteController(self::class, 'forum', $params);
		}
		...
		{
			case 'new_posts':
				return $this->rerouteController(self::class, 'newposts');
	...
	public function actionNewPosts(ParameterBag $params)
	{
		...
		/** @var FindNewPlugin $findNewPlugin */
		$findNewPlugin = $this->plugin(FindNewPlugin::class);
	...
	public function actionForum(ParameterBag $params)
	{
		...
		{
			$starterFilter = $this->em()->find(User::class, $filters['starter_id']);
		}
		else
		{
			$starterFilter = null;
	...
	protected function getAvailableForumSorts(Forum $forum)
	{
		return $forum->TypeHandler->getThreadListSortOptions($forum);
	}

	protected function getAvailableDateLimits(Forum $forum)
	{
		return [-1, 7, 14, 30, 60, 90, 182, 365];
	}

	protected function applyForumFilters(Forum $forum, ThreadFinder $threadFinder, array $filters)
	{
	...
	protected function applyDateLimitFilters(Forum $forum, ThreadFinder $threadFinder, array $filters)
	{
	...
	protected function getForumFilterInput(Forum $forum)
	{
		$filters = [];
	...
	protected function getPrefixIdIfUsable(Forum $forum): int
	{
	...
	protected function setupThreadCreate(Forum $forum)
	{
		...
		if ($setOptions)
		{
			...
			if (isset($setOptions['index_state']) && $thread->canEditModeratorFields())
			{
				$creator->setIndexState($this->filter('index_state', 'str'));
			}
		}
	...
	protected function finalizeThreadCreate(CreatorService $creator)
	{
		...
		if ($thread->canWatch())
		{
			...
			else
			{
				...
				$this->repository(ThreadWatchRepository::class)->autoWatchThread($thread, $visitor, true);
			}
		}
	...
	{
		return $this->rerouteController(self::class, 'postThread', $params);
	}
	...
	public function actionPostThreadChooser()
	{
		...
		$nodeTree = $nodeRepo->createNodeTree($nodes);
		$nodeTree = $nodeTree->filter(null, function ($id, Node $node, $depth, $children, Tree $tree)
		{
			if ($children)
			{
				return true;
	...
	public function actionPostThread(ParameterBag $params)
	{
		if (!$params->node_id && !$params->node_name)
		{
			return $this->rerouteController(self::class, 'postThreadChooser');
		}
		...
		if ($this->isPost())
		{
			...
			else
			{
				...
				if ($isPreRegThread)
				{
					/** @var PreRegActionPlugin $preRegActionPlugin */
					$preRegActionPlugin = $this->plugin(PreRegActionPlugin::class);
	...
	protected function getPreRegThreadActionData(CreatorService $creator)
	{
		$thread = $creator->getThread();
		$post = $creator->getPost();
	...
	public function actionPrefixHelp(ParameterBag $params)
	{
		...
		if ($prefixId)
		{
			$prefix = $this->em()->find(ThreadPrefix::class, $prefixId);
	...
	protected function getForumRss(?Forum $forum = null)
	{
	...
	protected function assertViewableForum($nodeIdOrName, array $extraWith = [])
	{
		...
		$finder = $this->em()->getFinder(ForumFinder::class);
		...
		$this->plugin(NodePlugin::class)->applyNodeContext($forum->Node);
	...
	{
		return $this->repository(ForumRepository::class);
	}
	 ...
	 */
	protected function getEffectiveSortInfo(Forum $forum, array $filters)
	{
		$sortInfo = [
