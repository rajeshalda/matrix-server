...
class EditHistoryController extends AbstractController
{
	public function actionIndex(ParameterBag $params)
	{
		...
		if ($oldId)
		{
			...
			/** @var EditHistory $old */
			$old = $this->assertRecordExists(EditHistory::class, $oldId);
			...
			if ($newId)
			{
				...
				/** @var EditHistory $new */
				$new = $this->assertRecordExists(EditHistory::class, $newId);
	...
	protected function assertHistoryViewable($historyId, array $extraWith = [])
	{
		...
		/** @var EditHistory $history */
		$history = $this->em()->find(EditHistory::class, $historyId, $extraWith);
		if (!$history)
		{
