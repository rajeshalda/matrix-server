...
{
	public const EXPORT_VERSION_ID = 2;
	...
	{
		$title = str_replace(' ', '-', Str::transliterate($this->style->title));
	...
	protected function getStyleNode(\DOMDocument $document)
	{
		...
		$styleNode->setAttribute('enable_variations', $style->enable_variations);
	...
	protected function getTemplateNode(\DOMDocument $document, array $template)
	{
		...
		$templateNode->appendChild(
			Xml::createDomCdataSection($document, $template['template'])
		);

		return $templateNode;
	}

	protected function getPropertyGroupNode(\DOMDocument $document, StylePropertyGroup $group)
	{
		$mapped = [
			'group_name',
			'title',
	...
	protected function getPropertyNode(\DOMDocument $document, StyleProperty $property, $addOnId)
	{
		$mapped = [
			'property_name',
			'group_name',
		...
		$propertyNode->appendChild(
			Xml::createDomElement($document, 'value', Json::jsonEncodePretty($property->property_value))
		);
	...
	protected function getExportableProperties()
	{
		...
		$properties = $this->em()->findByIds(StyleProperty::class, array_keys($results));
